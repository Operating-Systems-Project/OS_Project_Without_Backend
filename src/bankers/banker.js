...
function BuildFormFields($amount) {
	...
	for ($i = 0; $i < $amount; $i++) {
		...
		if ($i == 0) $field.innerHTML = "Resources";
		$item.appendChild($field);
		$field = document.createElement("input");
		$field.name = "Design[" + $i + "]";
		$field.type = "text";
		$field.setAttribute("class", "form-control");
		$field.placeholder = "R" + $i;
		$item.appendChild($field);
...
function BuildFormFields2($amount) {
	...
	for ($i = 0; $i < $amount; $i++) {
		...
		for ($j = 0; $j < res; $j++) {
			...
			$field = document.createElement("label");
			if ($j == 0) {
				$field.innerHTML = "Need for process " + $i;
			}
...
function BuildFormFields3($amount) {
	...
	for ($i = 0; $i < $amount; $i++) {
		for ($j = 0; $j < res; $j++) {
			...
			$field = document.createElement("label");
			if ($j == 0) {
				$field.innerHTML = "Allocation for process " + $i;
			}
...
function banker() {
	...
	while (1) {
		...
		li.setAttribute("class", "iter list-group-item list-group-item-info");
		...
		if (exec == pro) {
			...
			li.appendChild(document.createTextNode("Safe State -" + safeState));
			ul2.append(li);
			ul1.append(ul2);
			prints.appendChild(ul1);
			prints.appendChild(li);
